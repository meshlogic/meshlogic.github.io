...
class ExtraImageList_PT_ImagePreview(Panel):
    ...
    def draw(self, context):
        ...
        if img != None:
            ...
            #--- Clear image users
            row = layout.row(True)
            row = row.split(percentage=0.4, align=True)
            row.operator("extra_image_list.clear", text="Clear", icon='RADIO')                
            row.prop(props, "clear_mode", text="")
            row = layout.row()
...
class ExtraImageList_Props(bpy.types.PropertyGroup):
    ...
    clear_mode = EnumProperty(
        items = [
            ('SELECTED', "Selected Image", "Clear the image selected in the editor", 0),
            ('INVALID', "Invalid Images", "Clear invalid images (has_data == False)", 1),
            ('GENERATED', "Generated Images", "Clear generated images (source == 'GENERATED')", 2),
            ('BAKED', "Baked Images", "Clear images ending with a bake type e.g. '_COMBINED'", 3),
